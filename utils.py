...
def evaluate(normal_temp, normal_recon_temp, x_train, y_train, x_test, y_test, model, get_confidence=False, en_or_de=False):
    ...
    mu1_fit, sigma1_fit, mu2_fit, sigma2_fit = [float(v) for v in result.x] # Estimated parameter values
    sigma1_fit, sigma2_fit = abs(sigma1_fit), abs(sigma2_fit)  # Ensure positive std dev
    ...
    mu3_fit, sigma3_fit, mu4_fit, sigma4_fit = [float(v) for v in result.x] # Estimated parameter values
    sigma3_fit, sigma4_fit = abs(sigma3_fit), abs(sigma4_fit)  # Ensure positive std dev
